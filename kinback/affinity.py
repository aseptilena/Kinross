...
def invtf(mat, p): return tf(inverse(mat), p) # inverse transform
def translate(x, y = 0.): return (1., 0., 0., 1., float(x), float(y))
def rotate(th, o = None):
    if o == None: return (cos(th), sin(th), -sin(th), cos(th), 0., 0.)
    else: return compose(translate(o.real, o.imag), rotate(th), translate(-o.real, -o.imag))
def scale(x, y = None): return (float(x), 0., 0., float(x if y == None else y), 0., 0.)
def skew(x, y): pass # TODO (1, 0) -> (1, tan(x)); (0, 1) -> (tan(y), 1)
