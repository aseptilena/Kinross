...
class bezier:
    ...
    def __getitem__(self, n): return self.p[n]
    def __setitem__(self, n, v): self.p[n] = v
    ...
    def reverse(self):
        ...
        return bezier(*self[::-1])
    def isdegenerate(self):
        """True if all control points are coincident."""
        return min([isclose(self[-1], self[i]) for i in range(len(self.p) - 1)])
    
    def deriv(self):
        """The derivative of this Bézier curve."""
        return bezier(*[self.deg * (self[i + 1] - self[i]) for i in range(self.deg)])
    def velocity(self, t):
    ...
    def startdirc(self):
        N = 1
        while N <= self.deg:
            if not isclose(self[N], self[0]): return self[N] - self[0]
            N += 1
        return 1
    def enddirc(self):
        N = self.deg - 1
        while N >= 0:
            if not isclose(self[N], self[-1]): return self[N] - self[-1]
            N -= 1
        return -1
    ...
    def kind(self):
        ...
        if collinear(*self[:3]):
            if collinear(*self[1:]): return nothing
            p11, p01, p00 = self[1:]
            p4 = self[0]
        else:
            p00, p01, p11 = self[:3]
    ...
    def length(self, end = 1, start = 0):
        ...
        if self.deg == 1: return abs(self[1] - self[0])
